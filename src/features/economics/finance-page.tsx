...
import { ArrowDownCircle, ArrowUpCircle, Calculator, ChevronRight, CirclePlus, ReceiptText, Save, TrendingDown, TrendingUp, WalletCards, Wrench, X, UsersRound, type LucideIcon } from "lucide-react";
...
import { calculateCashFlowSummary, getCashFlowRange, type CashFlowPeriod, type CashFlowStatus } from "@/features/economics/cash-flow";
...
const formatMoney = (value: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(value);
const formatPricePerLiter = (value: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 4, maximumFractionDigits: 4 }).format(value);
...
const FinanceTabs = ({ section, onSelect }: { section: FinanceSection; onSelect: (section: FinanceSection) => void }) => {
  const tabs: Array<{ id: FinanceSection; label: string; icon: LucideIcon }> = [
    { id: "summary", label: "Resumen", icon: WalletCards },
    { id: "settlements", label: "Liquidaciones", icon: ReceiptText },
    { id: "movements", label: "Movimientos", icon: ArrowDownCircle },
    { id: "costs", label: "Costos", icon: Calculator }
  ];

  return <nav className="grid grid-cols-4 gap-1 rounded-2xl bg-stone-100 p-1.5" aria-label="Secciones de finanzas">
    {tabs.map((tab) => {
      const Icon = tab.icon;
      const active = section === tab.id;
      return <button key={tab.id} type="button" aria-pressed={active} className={`flex min-h-16 min-w-0 flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-center transition ${active ? "bg-white text-lime-950 shadow-sm" : "text-stone-600"}`} onClick={() => onSelect(tab.id)}><Icon size={18} aria-hidden="true" /><span className="w-full break-words text-[10px] font-black leading-3 sm:text-xs">{tab.label}</span></button>;
    })}
  </nav>;
};

const SectionLink = ({ title, description, icon: Icon, onClick, tone = "stone" }: { title: string; description: string; icon: LucideIcon; onClick: () => void; tone?: "stone" | "sky" }) => {
  const toneClasses = { stone: "border-stone-200 bg-white hover:bg-stone-50", sky: "border-sky-200 bg-sky-50 hover:bg-sky-100" };
  return <button type="button" aria-label={`Abrir ${title}`} className={`flex min-h-24 w-full items-center gap-3 rounded-3xl border p-4 text-left shadow-[0_8px_28px_rgba(28,25,23,0.05)] transition active:scale-[0.99] ${toneClasses[tone]}`} onClick={onClick}><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/90 text-lime-800 shadow-sm"><Icon size={22} aria-hidden="true" /></span><span className="min-w-0 flex-1"><span className="block text-lg font-black text-stone-950">{title}</span><span className="mt-1 block text-sm leading-snug text-stone-600">{description}</span></span><ChevronRight className="shrink-0 text-stone-400" size={21} aria-hidden="true" /></button>;
};
...
const EntryShell = ({ title, onClose, children }: PropsWithChildren<{ title: string; onClose: () => void }>) => (
  <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-50" role="dialog" aria-modal="true" aria-label={title}>
    <div className="mx-auto min-h-screen max-w-2xl p-4 pb-8 pt-[max(1rem,env(safe-area-inset-top))] sm:p-6">
      <header className="flex items-center justify-between gap-4 border-b border-stone-200 pb-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-lime-800">Finanzas</p><h1 className="mt-1 text-3xl font-black tracking-tight text-stone-950">{title}</h1></div><Button type="button" className="shrink-0 bg-white px-4 text-stone-800 ring-1 ring-stone-200" onClick={onClose} aria-label="Cerrar formulario"><X size={22} aria-hidden="true" /></Button></header>
...
const SettlementEntry = ({ session, farm, buyer, onSaved, onClose }: { session: FarmSession; farm: Farm | undefined; buyer: { id: string; name: string } | undefined; onSaved: (message: string) => void; onClose: () => void }) => {
  ...
  const save = async () => {
    if (!farm || !buyer) { setError("Primero completa el comprador de la finca en Configuración."); return; }
    try {
...
const MovementEntry = ({ session, onSaved, onClose }: { session: FarmSession; onSaved: (message: string) => void; onClose: () => void }) => {
  ...
  const [error, setError] = useState<string>();
  const save = async () => {
    try { await recordTransaction(db, { farmId: session.farmId, userId: session.userId, date, direction, category, amount: Number(amount), isEstimated: false }); onSaved("El movimiento quedó guardado en el celular."); }
...
const AssetEntry = ({ session, onSaved, onClose }: { session: FarmSession; onSaved: (message: string) => void; onClose: () => void }) => {
  ...
  const [error, setError] = useState<string>();
  const save = async () => {
    try { await recordAsset(db, { farmId: session.farmId, userId: session.userId, name, category: "Equipo", purchaseDate: date, purchaseValue: Number(value), usefulLifeYears: Number(years), salvageValue: 0 }); onSaved("El activo quedó guardado para calcular su depreciación."); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "No se pudo guardar el activo."); }
  };
...
const LaborEntry = ({ session, onSaved, onClose }: { session: FarmSession; onSaved: (message: string) => void; onClose: () => void }) => {
  ...
  const [error, setError] = useState<string>();
  const save = async () => {
    try { await recordLabor(db, { farmId: session.farmId, userId: session.userId, workerName, type: laborType, rate: Number(rate), daysWorked: Number(days), period: date.slice(0, 7) }); onSaved("El trabajo quedó guardado en el celular."); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "No se pudo guardar el trabajo."); }
  };
...
const statusCopy: Record<CashFlowStatus, { title: string; detail: string; className: string; icon: LucideIcon }> = {
  empty: { title: "Aún no hay caja registrada", detail: "Registra una liquidación o un movimiento para ver este período.", className: "bg-stone-900 text-white", icon: WalletCards },
  positive: { title: "Entró más de lo que salió", detail: "La caja registrada quedó a favor en este período.", className: "bg-lime-800 text-white", icon: TrendingUp },
  negative: { title: "Salió más de lo que entró", detail: "La caja registrada quedó en contra en este período.", className: "bg-red-800 text-white", icon: TrendingDown },
  balanced: { title: "La caja quedó equilibrada", detail: "Los ingresos y gastos registrados se compensan.", className: "bg-stone-800 text-white", icon: WalletCards }
};

const CashFlowDashboard = ({ summary, period, onPeriodChange, onShowCosts }: { summary: ReturnType<typeof calculateCashFlowSummary>; period: CashFlowPeriod; onPeriodChange: (period: CashFlowPeriod) => void; onShowCosts: () => void }) => {
  const periodOptions: Array<{ id: CashFlowPeriod; label: string }> = [{ id: "three_days", label: "3 días" }, { id: "week", label: "7 días" }, { id: "month", label: "30 días" }];
  const status = statusCopy[summary.status];
  const StatusIcon = status.icon;
  return <section className="space-y-4" aria-labelledby="cash-flow-title">
    <div className="flex items-center justify-between gap-3 px-1"><div><h2 id="cash-flow-title" className="text-2xl font-black tracking-tight text-stone-950">Caja de la finca</h2><p className="mt-0.5 text-sm font-semibold text-stone-500">{summary.range.label}</p></div><span className="rounded-full bg-stone-100 px-3 py-1.5 text-xs font-bold text-stone-600">{summary.activityCount} {summary.activityCount === 1 ? "registro" : "registros"}</span></div>
    <div className="grid grid-cols-3 gap-2 rounded-2xl bg-stone-100 p-1.5" role="group" aria-label="Período del resumen de caja">{periodOptions.map((option) => <button key={option.id} type="button" aria-pressed={period === option.id} className={`min-h-10 rounded-xl px-2 text-sm font-black transition ${period === option.id ? "bg-white text-lime-950 shadow-sm" : "text-stone-600"}`} onClick={() => onPeriodChange(option.id)}>{option.label}</button>)}</div>
    <section className={`rounded-3xl p-5 shadow-[0_14px_30px_rgba(28,25,23,0.14)] ${status.className}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold uppercase tracking-[0.14em] text-white/70">Resultado de caja</p><p className="mt-2 text-4xl font-black tracking-tight">{formatSignedMoney(summary.result)}</p></div><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/15"><StatusIcon size={23} aria-hidden="true" /></span></div><p className="mt-4 text-lg font-bold">{status.title}</p><p className="mt-1 text-sm leading-snug text-white/80">{status.detail}</p></section>
    <div className="grid grid-cols-2 gap-3"><section className="rounded-3xl border border-lime-200 bg-lime-50 p-4"><p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-lime-900"><ArrowUpCircle size={16} aria-hidden="true" />Ingresos</p><p className="mt-2 text-2xl font-black text-stone-950">{formatMoney(summary.totalIncome)}</p><p className="mt-1 text-xs font-semibold text-lime-900">Leche {formatMoney(summary.settlementIncome)}{summary.otherIncome > 0 ? ` · Otros ${formatMoney(summary.otherIncome)}` : ""}</p></section><section className="rounded-3xl border border-red-200 bg-red-50 p-4"><p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-red-900"><ArrowDownCircle size={16} aria-hidden="true" />Gastos</p><p className="mt-2 text-2xl font-black text-stone-950">{formatMoney(summary.totalExpenses)}</p><p className="mt-1 text-xs font-semibold text-red-900">Movimientos de egreso anotados</p></section></div>
    <Notice tone="info">Este resultado muestra la caja anotada: liquidaciones y movimientos del período. Para el costo completo por litro, que también considera depreciación y trabajo familiar, <button type="button" className="font-bold underline underline-offset-2" onClick={onShowCosts}>revisa Costos</button>.</Notice>
  </section>;
};

const SettlementHistory = ({ settlements, onRegister }: { settlements: Settlement[]; onRegister: () => void }) => (
  <section className="space-y-4"><header className="flex items-center justify-between gap-3 px-1"><h2 className="text-2xl font-black tracking-tight text-stone-950">Liquidaciones</h2><Button type="button" aria-label="Registrar liquidación" className="min-h-11 bg-lime-700 px-3 text-white sm:px-4" onClick={onRegister}><ReceiptText size={18} aria-hidden="true" /><span aria-hidden="true" className="hidden sm:inline">Registrar liquidación</span><span aria-hidden="true" className="sm:hidden">Registrar</span></Button></header>{settlements.length === 0 ? <Notice tone="info">Aún no hay liquidaciones registradas.</Notice> : <section className="overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-[0_8px_28px_rgba(28,25,23,0.06)]">{settlements.map((settlement, index) => <article key={settlement.id} className={`px-4 py-4 sm:px-5 ${index ? "border-t border-stone-100" : ""}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="font-black text-stone-950">{settlement.periodStart} al {settlement.periodEnd}</p><p className="mt-1 text-sm text-stone-600">{settlement.litersPaid.toFixed(1)} L · {formatPricePerLiter(settlement.pricePerLiterPaid)}/L</p></div><p className="shrink-0 text-lg font-black text-lime-800">{formatMoney(settlement.totalPaid)}</p></div>{settlement.legalPriceComputed !== undefined ? <p className={`mt-3 rounded-xl px-3 py-2 text-sm font-semibold ${settlement.legalVariancePerLiter && settlement.legalVariancePerLiter > 0 ? "bg-amber-50 text-amber-950" : "bg-lime-50 text-lime-950"}`}>Precio justo {formatPricePerLiter(settlement.legalPriceComputed)}/L · {settlement.legalVariancePerLiter && settlement.legalVariancePerLiter > 0 ? `${formatPricePerLiter(settlement.legalVariancePerLiter)}/L por debajo` : "pago coincide o supera el cálculo"}</p> : null}</article>)}</section>}</section>
);

const MovementHistory = ({ transactions, onRegister }: { transactions: Transaction[]; onRegister: () => void }) => (
  <section className="space-y-4"><header className="flex items-center justify-between gap-3 px-1"><h2 className="text-2xl font-black tracking-tight text-stone-950">Movimientos</h2><Button type="button" className="min-h-11 bg-lime-700 px-3 text-white sm:px-4" onClick={onRegister}><CirclePlus size={18} aria-hidden="true" />Registrar</Button></header>{transactions.length === 0 ? <Notice tone="info">Aún no hay movimientos registrados.</Notice> : <section className="overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-[0_8px_28px_rgba(28,25,23,0.06)]">{transactions.slice(0, 20).map((transaction, index) => <article key={transaction.id} className={`flex min-h-20 items-center gap-3 px-4 py-3 sm:px-5 ${index ? "border-t border-stone-100" : ""}`}><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl ${transaction.direction === "expense" ? "bg-red-50 text-red-800" : "bg-lime-50 text-lime-800"}`}>{transaction.direction === "expense" ? <ArrowDownCircle size={21} aria-hidden="true" /> : <ArrowUpCircle size={21} aria-hidden="true" />}</span><div className="min-w-0 flex-1"><p className="truncate font-black text-stone-950">{transaction.category}</p><p className="mt-0.5 text-sm text-stone-500">{transaction.date}</p></div><p className={`shrink-0 text-lg font-black ${transaction.direction === "expense" ? "text-red-800" : "text-lime-800"}`}>{transaction.direction === "expense" ? "−" : "+"}{formatMoney(transaction.amount)}</p></article>)}</section>}</section>
);
...
const CostsPage = ({ summary, calvesLiters, latestPrice, onAsset, onLabor }: { summary: ReturnType<typeof calculateCostSummary>; calvesLiters: number; latestPrice: number | undefined; onAsset: () => void; onLabor: () => void }) => (
  <section className="space-y-4"><header className="px-1"><h2 className="text-2xl font-black tracking-tight text-stone-950">Costo por litro</h2><p className="mt-1 text-sm leading-snug text-stone-600">Compara tres vistas para no confundir caja, depreciación y trabajo familiar.</p></header><div className="grid gap-3 sm:grid-cols-3"><Card><p className="text-sm font-bold uppercase tracking-wide text-stone-500">Caja</p><p className="mt-1 text-3xl font-black text-stone-950">{perLiter(summary.cashPerLiter)}</p></Card><Card><p className="text-sm font-bold uppercase tracking-wide text-stone-500">Con depreciación</p><p className="mt-1 text-3xl font-black text-stone-950">{perLiter(summary.withDepreciationPerLiter)}</p></Card><Card><p className="text-sm font-bold uppercase tracking-wide text-stone-500">Costo completo</p><p className="mt-1 text-3xl font-black text-stone-950">{perLiter(summary.fullPerLiter)}</p></Card></div>{calvesLiters > 0 ? <Notice tone="info">Leche para terneros: {calvesLiters.toFixed(1)} L{latestPrice ? ` · valor de referencia ${formatMoney(calvesLiters * latestPrice)} según la última liquidación.` : ". Registra una liquidación para calcular su valor de referencia."}</Notice> : null}<div className="grid gap-3 sm:grid-cols-2"><SectionLink title="Activo o equipo" description="Agrega equipos para incluir su depreciación." icon={Wrench} onClick={onAsset} /><SectionLink title="Trabajo" description="Registra trabajo familiar, jornal o mensual." icon={UsersRound} onClick={onLabor} tone="sky" /></div></section>
);
...
export const FinancePage = ({ session }: { session: FarmSession }) => {
  ...
  const cashFlow = useMemo(() => calculateCashFlowSummary(settlements, transactions, getCashFlowRange(nowInFarmTimezone().date, period)), [period, settlements, transactions]);
